...
export default function AccountPage() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-text">帳號設置</h1>
        <p className="text-sm text-text-sub mt-1">管理你的個人資料和安全設置</p>
      </div>
      ...
      <div className="bg-white border border-wine-border rounded-2xl p-6 flex items-center gap-5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-text-sub/60 mt-1">
            入駐日期：{new Date(account.joinDate).toLocaleDateString("zh-HK")}
          </p>
        </div>
      ...
      {account.role === "merchant" && (
        <Section title="危險操作">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-text">暫停帳號</p>
              <p className="text-xs text-text-sub mt-0.5">暫停後你的酒款將從比價列表中隱藏，可隨時重新開啟。</p>
            </div>
